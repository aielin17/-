...
const MUSIC = [
  {
    ...
    desc:'这一世英名我不要 只求换来红颜一笑',
    tags:['古风','情感'], fileType:'mp3', fileName:'红颜.mp3',
    file:'https://img.heliar.top/file/1772752066038_红颜.mp3',
    exportDate:'2025-01-01'
  },
  {
    id:'mus2', type:'music', name:'永远永远', author:'milk',
    desc:'你爱过我就已足够 就算到了最后爱已搁浅',
    tags:['情感','治愈'], fileType:'mp3', fileName:'永远永远.mp3',
...
const TUTORIALS = [
  {
    ...
    name: '全局主题 CSS 制作说明书',
    author: 'milk',
    desc: '从零开始定制你的专属界面。介绍全局主题 CSS 的核心变量、界面选择器和常用属性，附带可直接使用的三套完整模板及进阶技巧。',
    tags: ['主题', 'CSS', '入门', '模板'],
    content: `传 讯 · 全局主题 CSS 制作说明书
从零开始定制你的专属界面
...
全局主题 CSS 是一个让你用代码完全控制界面外观的功能。你可以把它理解为"给整个网页换皮肤"——从背景颜色、字体、气泡形状，到滚动条、弹窗、按钮，所有你能看到的东西都可以修改。

它与内置"主题编辑器"的区别：内置编辑器只能调颜色和几个预设选项；全局 CSS 没有限制，是真正的自由定制。

💡 就算完全不懂代码也没关系，本文档后半部分提供了可以直接复制使用的模板。
...
区块内有以下主要元素：
• 大文本框：在这里写或粘贴 CSS 代码
• 实时应用（勾选框）：勾选后每次在文本框里打字就会立刻看到效果，方便调试
• 应用 CSS 按钮：手动点击将代码保存并生效
• 清空按钮：清除所有已写的代码，恢复默认样式
...
选择器 = 你要修改谁（比如 .message-sent 表示"发送的气泡"）
属性   = 修改什么（比如 background-color 表示背景颜色）
值     = 改成什么样（比如 red 表示红色）
注释   = 灰色的 /* ... */ 部分，写给自己看的笔记，不会影响效果

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
四、核心 CSS 变量速查表
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

传讯界面里所有颜色都由以下变量控制。修改变量比直接改颜色更高效——改一个地方，全局生效。

变量名                    作用                             默认值（亮色）
--accent-color            强调色（按钮、链接、发送气泡）   #c5a47e
--accent-color-rgb        强调色的 R,G,B 数值              197, 164, 126
--primary-bg              主背景色（页面底色）             #f9f9f9
--secondary-bg            次级背景（卡片、输入框底色）     #ffffff
--text-primary            主要文字颜色                     #1a1a1a
--text-secondary          次要文字（时间戳、标签等）       #7a7a7a
--border-color            分割线和边框颜色                 #ebebeb
--header-bg               顶栏背景色                       #ffffff
--input-area-bg           底部输入区域背景色               #ffffff
--message-sent-bg         发送气泡背景色                   同 --accent-color
--message-sent-text       发送气泡文字颜色                 #ffffff
--message-received-bg     接收气泡背景色                   #f0f0f0
--message-received-text   接收气泡文字颜色                 #1a1a1a
--font-family             全局字体                         Noto Serif SC
--font-size               基础字号                         16px
--radius                  通用圆角大小                     16px
--shadow                  通用阴影                         0 4px 15px rgba(0,0,0,.06)

⚠ 修改 --accent-color 时，一定要同步修改 --accent-color-rgb，否则带透明度的元素颜色会对不上。

RGB 转换方法：#ff6b9d 的 RGB 是 255, 107, 157。可在浏览器地址栏搜索"HEX转RGB"在线工具获取。
...
选择器                 对应的界面元素
.message-sent          你发出的聊天气泡
.message-received      对方（或模拟对方）的气泡
.header                顶部栏整体
.header-inner          顶部栏内容区（头像+名字）
#message-input         底部消息输入框
.input-area            底部输入区整体
.input-area-wrapper    底部输入区外层包裹
#chat-container        聊天消息滚动区域
.modal-content         弹窗内容区（所有弹窗通用）
.modal-btn-primary     主要按钮（如"发送""保存"）
.modal-btn-secondary   次要按钮（如"取消""返回"）
.settings-card         设置页的每张功能卡片
.typing-indicator      "对方正在输入"提示条
::-webkit-scrollbar    滚动条（需 -webkit- 前缀）
body                   整个页面根元素
...
属性               作用                       示例值
background-color   背景颜色（纯色）           #1a1a2e
background         背景（可渐变/图片）        linear-gradient(135deg, #1a1a2e, #16213e)
color              文字颜色                   #e5e5e5
border-radius      圆角大小                   20px
border             边框样式                   1px solid rgba(255,255,255,0.1)
box-shadow         阴影效果                   0 4px 20px rgba(0,0,0,0.3)
font-family        字体                       "霞鹜文楷", serif
font-size          字号                       15px
padding            内边距                     12px 16px
backdrop-filter    毛玻璃效果                 blur(10px)
opacity            透明度（0～1）             0.85
...
:root {
  ...
  --message-received-bg: #1c2128;
  --message-received-text: #e6edf3;
}
.header {
  border-bottom: 1px solid rgba(127,166,205,0.2);
}
#message-input {
...
:root {
  --accent-color: #e8a0a8;
  --accent-color-rgb: 232, 160, 168;
  --primary-bg: #fdf6f0;
  --secondary-bg: #fff9f5;
  --text-primary: #3d2b1f;
  --text-secondary: #9e7b6e;
  --border-color: #f0ddd5;
  --message-received-bg: #faeee8;
  --message-received-text: #3d2b1f;
}
...
:root {
  --accent-color: #6a9b73;
  --accent-color-rgb: 106, 155, 115;
  --primary-bg: #f4f8f4;
  --secondary-bg: #eef5ee;
  --text-primary: #1f2e1f;
  --text-secondary: #6b876e;
  --border-color: #c8ddc9;
  --message-received-bg: #deeede;
  --message-received-text: #1f2e1f;
}
...
【8.3 深色模式单独控制】
当用户开启深色模式时，html 元素会带上 data-theme="dark" 属性，可以单独针对它写样式：
html[data-theme="dark"] #message-input {
  background: rgba(255,255,255,0.06) !important;
}

【8.4 !important 的使用】
如果你修改了某个属性但没有生效，可以在值后面加上 !important 来强制覆盖：
.message-sent { background-color: #ff6b9d !important; }
⚠ !important 应尽量少用，只在正常方式不生效时才用。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
九、常见问题
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Q：修改了颜色，但强调色相关的元素颜色不对？
A：检查是否同步修改了 --accent-color-rgb。两个必须保持一致，否则带透明度的元素会用旧的 RGB 值计算。
...
Q：想恢复默认样式怎么办？
点击"清空"按钮，然后再点"应用 CSS"即可完全恢复。

Q：可以导入导出主题吗？
可以。全局 CSS 代码会跟随设置存储。在"数据管理"中导出备份时，CSS 代码也会包含在内。

--- 遇到问题可以截图请 AI 帮你调整，详见《AI 制作说明书》---`
  },
  {
    id: 'tut2',
    type: 'tutorial',
    name: '请 AI 制作主题 CSS 说明书',
    author: 'milk',
    desc: '手把手教你如何把提示词发给 AI，让它直接生成可粘贴使用的完整主题 CSS 代码，附带可直接复制的提示词模板及调整技巧。',
    tags: ['主题', 'AI', '提示词', '教程'],
    content: `传 讯 · 请求 AI 制作主题 CSS
给 AI 看的技术参考文档
...
把以下"发给 AI 的提示词模板"部分直接复制给 AI（如 Claude、ChatGPT 等），然后在末尾描述你想要的风格即可。

提示词越具体越好，例如："深色背景、紫色强调色、气泡要有磨砂玻璃效果、字体大一点"

如果你有参考图或者喜欢的网站，可以描述给 AI 或者把截图发给 AI，AI 会从视觉风格里提取配色。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【直接复制给 AI 的提示词模板】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

以下内容为发给 AI 的起始提示词，请从下一行开始复制：

请帮我为"传讯"这个聊天工具制作一段全局主题 CSS。以下是这个工具的技术规格，请严格按照这些规格输出代码：
...
【1. CSS 注入方式】
代码通过 <style id="user-custom-global-theme-style"> 标签注入到 <head>，作用于整个页面。支持所有标准 CSS 语法，包括 CSS 变量、伪类、媒体查询、动画等。

【2. 核心 CSS 变量（必须优先修改 :root 里的这些变量）】

--accent-color        强调色（按钮、发送气泡背景、链接等）
--accent-color-rgb    强调色的 R,G,B 数值，格式：255, 107, 157
                      ⚠ 必须与 --accent-color 的颜色保持一致！
--primary-bg          主背景色（页面底色）
--secondary-bg        次级背景（卡片、输入框底色）
--text-primary        主要文字颜色
--text-secondary      次要文字（时间戳、标签等）
--border-color        分割线和边框颜色
--header-bg           顶部栏背景色
--input-area-bg       底部输入区域背景色
--message-sent-bg     发送气泡背景（默认同 --accent-color）
--message-sent-text   发送气泡文字颜色
--message-received-bg      接收气泡背景色
--message-received-text    接收气泡文字颜色
--font-family         全局字体
--font-size           基础字号
--radius              通用圆角
--shadow              通用阴影
...
.message-sent          发送气泡
.message-received      接收气泡
.header                顶部栏整体
.header-inner          顶部栏内容区（头像+名字区域）
#message-input         消息输入框 textarea
.input-area            底部输入区整体
.input-area-wrapper    底部输入区外层包裹
#chat-container        聊天消息滚动容器
.modal-content         所有弹窗内容区
.modal-btn-primary     主要按钮（发送/保存等）
.modal-btn-secondary   次要按钮（取消/返回等）
.settings-card         设置页功能卡片
.typing-indicator      "正在输入"提示条
body                   整个页面根元素

【4. 亮色/深色模式区分】

亮色模式：html（无额外 attribute）
深色模式：html[data-theme="dark"]

如需为深色模式单独指定颜色，使用：
html[data-theme="dark"] { --primary-bg: #121212; ... }

【5. 输出格式要求】

- 只输出 CSS 代码，不要输出任何说明文字
- 代码从 :root { 开始
- 要包含亮色和深色两套变量定义
- 使用清晰的注释分隔各个部分
- 不要使用任何需要额外加载的外部字体（除非我明确要求）
- 如果要使用透明效果，请确保对比度仍然可读
...
（在这里填写你想要的风格描述，例如：）
深色背景、以深紫色为主调、气泡有轻微毛玻璃效果、文字偏细偏柔、整体像夜晚星空的感觉

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
进阶：已有代码，让 AI 帮你调整
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

如果 AI 已经给了你一段代码，你想微调，可以用以下提示词格式：

  这是我目前的传讯主题 CSS 代码：[粘贴你的代码]
  请帮我调整：[描述你想改的地方]
  注意不要破坏已有的整体风格，只修改指定部分。
...
1. 复制 AI 给的全部 CSS 代码（通常以 :root { 开头）
2. 打开传讯 → 设置 → 外观设置 → 气泡 & CSS → 全局主题 CSS
3. 把复制的代码粘贴到文本框中
4. 勾选"实时应用"看效果预览（可以不点保存就先看）
5. 满意后点击"应用 CSS"按钮正式保存

如果效果不对或者某些地方看起来奇怪，截图给 AI 并描述问题，AI 可以帮你修复。
...
--- 传讯 · 让每一次对话都有温度 ---`
  },
  {
    ...
    name: '气泡 CSS 定制说明书',
    author: 'milk',
    desc: '🎨 适用于聊天应用·自定义气泡 & CSS 面板。介绍气泡 HTML 结构、选择器、常用属性，含少女粉、赛博朋克、毛玻璃三套完整示例代码，发给 AI 即可生成专属气泡方案。',
    tags: ['气泡', 'CSS', 'AI', '示例代码'],
    content: `🎨 气泡样式 CSS 定制说明书
适用于聊天应用·自定义气泡 & CSS 面板
...
聊天界面中，每条消息的气泡外形、颜色、圆角、字体等，均可通过自定义 CSS 代码来修改。只需把 CSS 代码粘贴进「气泡 & CSS」面板的输入框，刷新后即刻生效。

本说明书的用途：把此文档发给 AI（如 Claude、ChatGPT 等），AI 就能了解你的应用结构，直接帮你生成可以粘贴使用的完整 CSS 代码。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
二、HTML 结构速查
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

在生成 CSS 前，AI 需要知道气泡使用哪些 CSS 类名。以下是核心结构：

【2.1 消息容器】
.message-wrapper          /* 单条消息的外层包裹（含头像 + 气泡）*/
.message-wrapper.sent     /* 我方消息 */
.message-wrapper.received /* 对方消息 */

【2.2 气泡本体】
.message-bubble           /* 气泡本体（所有消息通用）*/
.sent .message-bubble     /* 我方气泡 */
.received .message-bubble /* 对方气泡 */

【2.3 消息文字】
.message-text             /* 气泡内的文字 */
.message-time             /* 时间戳 */
.message-sender-name      /* 显示名称（群聊可见）*/

【2.4 气泡尾巴（三角）】
气泡三角通过伪元素 ::before / ::after 绘制，建议配合 border-radius 和 clip-path 使用。
.sent .message-bubble::before     /* 我方气泡尾巴 */
.received .message-bubble::after  /* 对方气泡尾巴 */

【2.5 CSS 变量（主题色）】
应用使用 CSS 变量统一控制主题色，气泡 CSS 可直接引用，跟随主题自动变色：

--accent-color         主题强调色（如金色/蓝色等）
--accent-color-rgb     同上，但 RGB 格式（用于 rgba()）
--message-sent-bg      我方气泡默认背景色
--message-received-bg  对方气泡默认背景色
--text-primary         主文字颜色
--text-secondary       次级文字颜色
--border-color         边框颜色
--secondary-bg         次级背景色
--radius               全局圆角大小（默认 18px）
--shadow               全局阴影
...
属性               作用说明                       示例值
background         气泡背景色（可用渐变）         #FFE4F0 或 linear-gradient(...)
border-radius      气泡四角圆角大小               18px 或 20px 4px 20px 20px
border             气泡边框线                     1px solid rgba(0,0,0,0.1)
box-shadow         气泡投影效果                   0 2px 12px rgba(0,0,0,0.12)
color              文字颜色                       #333 或 var(--text-primary)
font-size          文字大小                       14px / 15px / 16px
font-weight        字重（粗细）                   400（常规）/ 600（加粗）
padding            气泡内边距                     10px 14px
max-width          气泡最大宽度                   75% / 280px
backdrop-filter    毛玻璃效果（需透明背景）        blur(10px)
opacity            整体透明度（0～1）              0.95
letter-spacing     字间距                         0.3px
line-height        行高                           1.6
transition         过渡动画（hover效果）          all 0.2s ease

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
四、发给 AI 的提示词模板
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

将以下提示词（根据你的需求修改后）发给 AI，附带本文档或将内容粘贴过去：

📋 提示词模板（直接复制使用）

请根据以下 CSS 结构说明，为我生成一套【风格描述】的聊天气泡 CSS 代码。

要求：
• 我方气泡（.sent .message-bubble）：[颜色/风格要求]
• 对方气泡（.received .message-bubble）：[颜色/风格要求]
• 整体风格：[例如：少女感 / 赛博朋克 / 极简白 / 毛玻璃 / 暖橙]
• 字体颜色：[深色 / 浅色 / 跟随主题]
• 是否需要气泡尾巴：[需要 / 不需要]
• 其他特效：[例如：悬停加深、微动画等]

请只返回完整可用的 CSS 代码，放在代码块内。
...
/* 我方气泡 - 粉色渐变 */
.sent .message-bubble {
  background: linear-gradient(135deg, #FFBCD9, #FFE0ED);
  border-radius: 20px 4px 20px 20px;
  box-shadow: 0 3px 14px rgba(255,100,150,0.18);
...
.sent .message-bubble {
  ...
  box-shadow: 0 0 18px rgba(0, 200, 255, 0.25), inset 0 0 8px rgba(0,200,255,0.05);
  color: #a0e8ff;
}

/* 对方气泡 - 暗底紫光 */
.received .message-bubble {
  background: rgba(18, 8, 35, 0.90);
...
.received .message-bubble {
  ...
  box-shadow: 0 4px 16px rgba(0,80,200,0.1);
  color: #001a40;
}
...
• 将生成好的 CSS 粘贴进「外观设置 → 气泡 & CSS → 自定义 CSS」文本框，无需手动保存，实时生效。
• 建议先备份当前 CSS（复制出来保存），再粘贴新代码，方便比对和回退。
• 如需使用毛玻璃效果（backdrop-filter），需要聊天背景非纯色（即设置了背景图），否则看不出效果。
• 气泡尾巴（三角）的精细调节较复杂，可告诉 AI「不需要气泡尾巴」来隐藏默认三角。
• 某些效果（如 box-shadow 发光）在深色主题下更明显，浅色主题下可能需要减小透明度。
• 如果气泡文字颜色不对，优先检查 color 属性是否被覆盖，可用 color: #333 !important 强制生效。

本说明书由应用自动配套生成，可直接发给 AI 助手使用 ✨`
  }
];

const ALL = [...BUBBLES, ...FONTS, ...CARDS, ...THEMES, ...MUSIC, ...TUTORIALS];
