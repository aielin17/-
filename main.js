...
function loadFontFavs() {
  try {
    return JSON.parse(localStorage.getItem(LS_FONT_FAV) || '[]');
  } catch (e) {
    return [];
  }
}

function saveFontFavs(arr) {
  try {
    localStorage.setItem(LS_FONT_FAV, JSON.stringify(arr));
  } catch (e) {}
}
...
function renderAuthors() {
  const authorMap = {};
  ALL.forEach(i => {
    authorMap[i.author] = (authorMap[i.author] || 0) + 1;
  });
...
function renderBubbleModal(item) {
  ...
  defBtn.onclick = () => {
    modalState.fontFamily = '';
    body.style.fontFamily = '';
    saveFont('');
    updateFontBtns(footer, modalState);
  };
  ...
  FONTS.filter(f => DEFAULT_FONT_IDS.includes(f.id)).forEach(f => {
    ...
    btn.onclick = () => {
      modalState.fontFamily = f.family;
...
window.copyFallback = function(type) {
  copyText(document.getElementById('fb-content-' + type).value, '投稿内容');
};
